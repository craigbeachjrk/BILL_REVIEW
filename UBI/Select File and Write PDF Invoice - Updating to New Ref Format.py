...
def get_misc_file_name(title):
    root = Tk()
    root.withdraw()
    user_input = simpledialog.askstring("Input", title)
    root.destroy()
...
if __name__ == "__main__":
    try:
        ...
        # Get miscellaneous file name component
        misc_file_name = get_misc_file_name("What do you want to add to the end of the FILE name? Leave blank if nothing. Lead with a space and a dash ( -) if something.")
        ...
        for _, group_data in invoice_groups:
            ...
            prop_attn       = group_data["prop_attn"].iloc[0]
            prop_add1       = group_data["prop_remit_add1"].iloc[0]
            prop_add2       = group_data["prop_remit_csz"].iloc[0]
            prop_phone      = group_data["prop_phone"].iloc[0]
            prop_delinquent = group_data["prop_msg1"].iloc[0]
            prop_not_a_bill = group_data["prop_msg2"].iloc[0]
        
            # Utility charges
            gas       = group_data["gas"].iloc[0]
            ubilling   = group_data["UBILL"].iloc[0]
            ...
            ubilling = 0 if pd.isna(ubilling) else ubilling
            ...
            total_charges = sum([
                ...
                trash, elec, pestc, kvfd, storm, env, hoa, ubi_deposit, ubilling
            ])
            ...
            pdf_filename = f"UBI-{p_i}-{lease_id}-{safe_account_num}-{safe_bill_start}{misc_file_name}.pdf"
