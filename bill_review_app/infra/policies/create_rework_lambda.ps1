...
$trustObj | ConvertTo-Json -Depth 5 | Set-Content -Path $trustPath -Encoding UTF8
...
$policyObj | ConvertTo-Json -Depth 8 | Set-Content -Path $policyPath -Encoding UTF8
...
$envObj = @{ Variables = @{ BUCKET=$Bucket; REWORK_PREFIX=$ReworkPrefix; ENRICH_PREFIX=$EnrichPrefix; POST_PARSED_PREFIX=$PostParsedPrefix; PRE_ENTRATA_PREFIX=$PreEntrataPrefix } } | ConvertTo-Json -Depth 3 -Compress
aws lambda update-function-configuration --function-name $LambdaName --region $Region --profile $Profile --environment $envObj | Out-Null
...
$outNotif = Join-Path $Temp 'notif.json'
$notif | ConvertTo-Json -Depth 8 | Set-Content -Path $outNotif -Encoding UTF8
aws s3api put-bucket-notification-configuration --bucket $Bucket --notification-configuration file://$outNotif --region $Region --profile $Profile | Out-Null
