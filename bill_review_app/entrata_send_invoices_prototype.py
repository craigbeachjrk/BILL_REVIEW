...
def _normalize_location(loc_item) -> Dict[str, str]:
    ...
    if isinstance(loc_item, dict):
        ...
        # Prefer "locationName" (flat vendor-location records) over "name" (which is vendor name in flat format)
        lname = str(loc_item.get("locationName") or loc_item.get("name") or "").strip()
...
def _parse_vendor_data(data) -> Dict[str, List[Dict[str, str]]]:
    ...
    if isinstance(data, dict):
        ...
        if isinstance(data.get("vendors"), list):
            for it in data["vendors"]:
                ...
                if not vid:
                    continue
                # Single location (flat vendor-location record)
                loc = it.get("vendorLocationId") or it.get("VendorLocationId") or it.get("locationId")
                if loc:
                    # Pass the full record to preserve locationName
                    out.setdefault(vid, []).append(_normalize_location(it))
                # Multiple locations (nested structure)
    ...
    if isinstance(data, list):
        for it in data:
            ...
            if not vid:
                continue
            # Check if this is a flat vendor-location record (has locationId at top level)
            loc = it.get("vendorLocationId") or it.get("VendorLocationId") or it.get("locationId")
            if loc:
                # Pass the full record to preserve locationName
                out.setdefault(vid, []).append(_normalize_location(it))
...
def build_send_invoices_payload(rows: List[Dict[str, Any]], vendor_loc_resolver, post_month_date: Optional[str] = None) -> Dict[str, Any]:
...
def do_post(payload: Dict[str, Any], dry_run: bool = False) -> Tuple[bool, str]:
    ...
    # Budget: AppRunner kills requests at 120s. Bills post sequentially, so
    # worst-case per bill = retries × (connect + read timeout + backoff).
    # 2 retries × (10+15+1) = ~52s per bill → 2 bills = ~104s, safe under 120s.
    max_retries = 2
    base_timeout = 15
    ...
    for attempt in range(max_retries):
        try:
            ...
            last_error = f"Timeout after {base_timeout}s (attempt {attempt + 1}/{max_retries})"
            if attempt < max_retries - 1:
                import time
                time.sleep(2 ** attempt)  # Exponential backoff: 1s
                continue
