...
def _norm_bf(s: str) -> str:
    try:
        ...
        return str(s or "")
def write_ndjson(bucket: str, key_stem: str, rows: list[list[str]], source_input_key: str, bill_from: str = "", pdf_id: str = ""):
    ndjson_lines = []
    ...
    def fmt_us_date(s: str) -> str:
        ...
        import re as _re; digits = _re.sub(r"\D", "", s)
        try:
            if len(digits) == 8:
...
def lambda_handler(event, context):
    ...
    for record in event.get("Records", []):
        ...
        try:
            ...
            pass
        # Delete source file from queue (Pending or Standard) now that it's copied to Parsed_Inputs
        if is_pending or is_standard:
            try:
                s3.delete_object(Bucket=bucket, Key=key)
                queue_name = "Pending" if is_pending else "Standard"
                print(json.dumps({"message": f"Deleted from {queue_name}", "key": key}))
            except Exception:
                pass
